...
const createCategory = async (req, res) => {
	...
	try {
		const { name } = req.body;
		if (!name) {
			return res.status(400).json({ message: "Invalid field", error: "No name specified" });
		}

		const categoryList = await db.Category.findOne({ where: { [Op.and]: { user_id: userUUID, name: { [Op.iLike]: name } } } });
		if (categoryList) {
			return res.status(400).json({ message: "Invalid field", error: "Category already exists" });
		}
...
const updateCategory = async (req, res) => {
	...
	try {
		const { id } = req.params;
		if (isNotUUID(id)) {
			return res.status(400).json({ message: "Invalid field", error: "Incorrect ID format" });
		}
...
const deleteCategory = async (req, res) => {
	...
	try {
		...
		if (categoryExpenseList && !booleanForce) {
			return res.status(400).json({ message: "Cannot delete", error: "Category is still used" });
		}
